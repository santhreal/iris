...
| **Clipboard** | `arboard` on the `CLIPBOARD` selection (images, text, files as `text/uri-list`) | `arboard` over `ext-data-control-v1` or `wlr-data-control-unstable-v1`, else XWayland (images, text, files as `text/uri-list`) | `arboard` (images); `CF_HDROP` (files) | `arboard` (images); `NSPasteboard` `NSURL` (files) |
...
- **Drag-Out and Clipboard**: Drag-out speaks the XDND protocol from a private 1x1 window tracking the pointer via `XQueryPointer`. Clipboard copies (images as `image/png`, text as `UTF8_STRING`, and files as `text/uri-list`) run through one `arboard` clipboard, which the daemon opens on its first copy and keeps. It serves the `CLIPBOARD` selection until another client takes it.
...
- **X11 Connection**: None at startup, with `DISPLAY` set or not, so the daemon starts no on-demand XWayland. The Vulkan instance enables the Wayland surface extension alone; Mesa's device selection layer queries an X server only for an instance that enables the XCB one. The daemon creates the instance with `DISPLAY` removed from its environment, since the NVIDIA driver connects to that X server when the Vulkan loader loads it, and restores the variable before it enumerates GPUs.
- **System Tray**: Registers a StatusNotifierItem interface using `ksni`.
...
- **Drag-Out and Clipboard**: File drag-out is unsupported. Clipboard copies (images as `image/png`, text, and files as `text/uri-list`) run through `arboard`, which sets the selection through the compositor's `ext-data-control-v1` or `wlr-data-control-unstable-v1` protocol with no iris window focused and no X connection. On a compositor that offers neither, a copy goes through the X server in `DISPLAY` (XWayland), and the first copy can start an on-demand XWayland.
- **Reveal in Folder**: Calls DBus method `org.freedesktop.FileManager1.ShowItems` via `dbus-send`, falling back to `xdg-open`.
